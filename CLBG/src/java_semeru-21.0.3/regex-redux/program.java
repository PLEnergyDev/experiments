/*
   ...
   modified for JVM compatibility
*/
...
public class program {
    ...
    private static void run_benchmark() throws Throwable {
        ...
        try (Arena arena = Arena.ofShared()) {
            ...
            var rawInputBuffer = arena.allocate(initialLength);
            rawInputBuffer.asByteBuffer().put(rawInput);
            ...
            // Process the regex counting tasks
    ...
    private static int substitute(
        ...
        try (Arena arena = Arena.ofConfined()) {
            var replacementBytes = replacement.getBytes(StandardCharsets.US_ASCII);
            var replacementBuffer = arena.allocate(replacementBytes.length);
            replacementBuffer.asByteBuffer().put(replacementBytes);
    ...
    private static MemorySegment compilePattern(String pattern) throws Throwable {
        try (Arena arena = Arena.ofConfined()) {
            ...
            var bufPattern = arena.allocate(patternBytes.length);
            bufPattern.asByteBuffer().put(patternBytes);
            ...
            var compiledPattern = (MemorySegment) pcre2_compile_8.invoke(
                    bufPattern, patternBytes.length, 0,

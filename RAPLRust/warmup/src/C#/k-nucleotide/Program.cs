...
public static class Program {
    ...
    public static void Main(string[] args) {
        ...
        // Read input once and store the result
        var buffer = GetBytesForThirdSequence();

        for (int counter = 0; counter < count; counter++) {
            start_rapl();
            PrepareLookups();
            var fragmentLengths = new [] {
                1,
                2,
                3,
                4,
            ...
            var dicts =
                (from fragmentLength in fragmentLengths.AsParallel() select CountFrequency(buffer, fragmentLength)).ToArray();
    ...
    private static void WriteFrequencies(Dictionary < ulong, Wrapper > freq, int buflen, int fragmentLength) {
        ...
        foreach(var line in (from k in freq.Keys orderby freq[k].V descending select string.Format("{0} {1:f3}", PrintKey(k, fragmentLength),
    ...
    private static Dictionary < ulong, Wrapper > CountFrequency(byte[] buffer, int fragmentLength) {
        var dictionary = new Dictionary < ulong,
            Wrapper > ();
        ulong rollingKey = 0;
        ulong mask = 0;
        int cursor;
        ...
        byte cursorByte;
        while (cursor < stop) {
            if ((cursorByte = buffer[cursor++]) < (byte)
                'a')
    ...
    // This method reads the input from stdin and stores the bytes for the third sequence.
    private static byte[] GetBytesForThirdSequence() {
        ...
        while (!threeFound && (amountRead = source.Read(buffer, 0, buffersize)) > 0) {
            ...
            threeFound = (indexOfGreaterThan > -1 &&
                buffer[indexOfGreaterThan + 1] == (byte)
                'T' &&
                buffer[indexOfGreaterThan + 2] == (byte)
                'H');
            if (threeFound) {

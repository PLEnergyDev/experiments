...
char const *spell_number(size_t n) {
  static char buf[128];
  static char const *numbers[] = {" zero", " one", " two",   " three", " four",
...
void init_and_start(enum color_t *initial_colors, size_t chameneos_count,
                    struct meeting_place_t **place,
                    struct chameneos_t ***chameneos, size_t meet_count,
                    int is_smp, cpu_set_t *affinity) {
  size_t i;

  place[0] = (struct meeting_place_t *)cache_aligned_malloc(
      sizeof(struct meeting_place_t));
  place[0]->state = meet_count << MEET_COUNT_SHIFT;
  chameneos[0] = (struct chameneos_t **)cache_aligned_malloc(
      chameneos_count * sizeof(struct chameneos_t *));
  for (i = 0; i != chameneos_count; i += 1) {
    chameneos[0][i] =
        (struct chameneos_t *)cache_aligned_malloc(sizeof(struct chameneos_t));
    ...
    if (pthread_create(&chameneos[0][i]->thread, &chameneos[0][i]->thread_attr,
                       chameneos_func, chameneos[0][i]))
      exit(1);
  }
}
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < count; i++) {
    start_rapl();
    enum color_t initial_colors1[] = {color_blue, color_red, color_yellow};
    ...
    if (is_smp) {
      init_and_start(initial_colors1, chameneos_count1, &place1, &chameneos1,
                     meet_count, is_smp, &affinity1);
      init_and_start(initial_colors2, chameneos_count2, &place2, &chameneos2,
                     meet_count, is_smp, &affinity2);

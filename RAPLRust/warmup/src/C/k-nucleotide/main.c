...
int main(int argc, char **argv) {
  ...
  // Read the input polynucleotide data once before the loop.
  char buffer[4096];
  ...
  // Read and encode the third polynucleotide.
  while (fgets(buffer, sizeof(buffer), stdin) && buffer[0] != '>') {
    ...
    // Make sure we still have enough memory allocated.
  ...
  // Now perform the loop over multiple iterations.

...
// root node.
static inline intnative_t
compute_Tree_Checksum(const tree_node *const root_Node) {
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    intnative_t maximum_Tree_Depth = atoi(argv[2]);
...
#pragma omp parallel for
    for (current_Tree_Depth = minimum_Tree_Depth;
      ...
      intnative_t iterations =
          1 << (maximum_Tree_Depth - current_Tree_Depth + minimum_Tree_Depth);
      ...
      for (; i <= iterations; ++i) {
        ...
        tree_node *const tree_1 =
            create_Tree(current_Tree_Depth, thread_Memory_Pool);

...
lib_path = "../../rapl-interface/target/release/librapl_lib.so"
...
def main():
    # Assuming you need to handle some subprocess output (for example, if you want to compile something)
    pipes = subprocess.Popen(
        ["your-command-here"],  # Replace with the actual command
    ...
    # Handle `std_out` as binary data, without decoding it
    with open('output_file.pbm', 'wb') as f:
        f.write(std_out)

    # Alternatively, you can process it directly:
    # process_binary_output(std_out)

if __name__ == '__main__':
    for i in range(count):
        dll.start_rapl()
        if len(argv) > 1:
            mandelbrot(int(argv[2]))
        else:
            main()
        dll.stop_rapl()

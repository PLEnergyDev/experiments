...
string SpellNumber(int n) {
  static const char *numbers[] = {" zero", " one", " two",   " three", " four",
...
struct Creature {
  ...
  void Wait() const { pthread_join(threadHandle, NULL); }
...
struct MeetingPlace {
  // max # of creatures is ( 1 << S ) - 1, max # of meetings is ( 1 << ( 32 - S
  // ) ) - 1
  ...
  void MeetUp(Creature *creature) {
    ...
    while (true) {
      ...
      if (waiting)
        ...
        // nobody waiting and meetings left, set ourselves to the waiting
        // creature
...
protected:
  volatile int state; // state is read & set from other threads, don't cache in
                      // a register
  int idGenerator;
  Creature **creatures;
};
...
void Creature::Run() { place->MeetUp(this); }

template <int ncolor> struct Game {
...
int main(int argc, const char *argv[]) {
  ...
  for (int counter = 0; counter < count; counter++) {
    ...
    const Color r2[] = {blue, red, yellow, red, yellow,
                        blue, red, yellow, red, blue};

    for (int c1 = blue; c1 <= yellow; c1++)
      for (int c2 = blue; c2 <= yellow; c2++)
        cout << r1[c1] << " + " << r1[c2] << " -> " << (r1[c1] + r1[c2])
             << endl;
    cout << endl;

    int n = (argc >= 2) ? atoi(argv[2]) : 6000000;

...
//  -pipe -Wall -O3  -fomit-frame-pointer -march=native -mfpmath=sse -msse2
//  -fopenmp   -mno-fma --std=c++14
...
bool vec_le(double *v, double f) {
  return (v[0] <= f || v[1] <= f || v[2] <= f || v[3] <= f || v[4] <= f ||
          v[5] <= f || v[6] <= f || v[7] <= f);
}
...
// up computations.
//
inline void calcSum(double *r, double *i, double *sum, double const *init_r,
                    Vec const &init_i) {
  auto r_v = (Vec *)r;
  auto i_v = (Vec *)i;
  auto sum_v = (Vec *)sum;

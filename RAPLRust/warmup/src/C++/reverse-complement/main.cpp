...
struct ReverseLookup {
  ...
  short operator[](const short &s) {
    return (short)wordLookup[(unsigned short)s];
  }
...
template <class type> struct vector2 : public std::vector<type> {
...
struct Chunker {
  ...
  template <int N> struct LinePrinter {
...
struct ReverseComplement {
  ReverseComplement() : data(nullptr) {}

  ~ReverseComplement() { delete[] data; }

  void ReadInput() {
    // get stdin file size (read only once)
    if (data == nullptr) {
...
int main(int argc, const char *argv[]) {
  ...
  revcom.ReadInput(); // Read input once

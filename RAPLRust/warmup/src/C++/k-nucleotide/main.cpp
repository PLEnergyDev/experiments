...
struct T {
  ...
  bool operator<(const T &in) const { return data < in.data; }
  bool operator==(const T &in) const { return data == in.data; }
...
calculate(const std::string &input, unsigned size, unsigned beg = 0,
          unsigned incr = 1) {
  __gnu_pbds::cc_hash_table<T, unsigned, T::hash> frequencies;
  T tmp;
  for (unsigned i = beg, i_end = input.size() + 1 - size; i < i_end;
       i += incr) {
...
tcalculate(const std::string &input, unsigned size) {
  ...
  for (unsigned i = 0; i < N; ++i)
    ft[i] =
        std::async(std::launch::async, calculate, std::ref(input), size, i, N);

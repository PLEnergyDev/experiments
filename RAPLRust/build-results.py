...
def compile(benchmark):
    for lang in BENCHMARK_LANGUAGES:
        lang_dir = os.path.join(benchmark, "results", lang)
...
def average_results(benchmark, skip=15):
    averaged_dir = os.path.join(benchmark, "results", "averaged")
    if os.path.exists(averaged_dir):
        shutil.rmtree(averaged_dir)
    os.mkdir(averaged_dir)

    for lang in BENCHMARK_LANGUAGES:
        lang_results = os.path.join(benchmark, "results", lang, "rapl.csv")
        if not os.path.exists(lang_results):
            print(
        ...
        for bench in BENCHMARKS:
            ...
            averaged_results = os.path.join(
                benchmark, "results", "averaged", f"{bench}.csv"
            )
...
def normalize_results(benchmark, skip=15):
    ...
    time = 0
    normalized_dict = {}
    normalized_result = os.path.join(benchmark, "results", "normalized.csv")
    for lang in BENCHMARK_LANGUAGES:
        lang_results = os.path.join(benchmark, "results", lang, "rapl.csv")
        if not os.path.exists(lang_results):
            print(
...
def dendrograms(benchmark):
    ...
    plots_dir = os.path.join(benchmark, "results", "plots", "dendrograms")
...
def violins(benchmark):
    plots_dir = os.path.join(benchmark, "results", "plots", "violins")
    if not os.path.exists(plots_dir):
        os.makedirs(plots_dir)
    ...
    for lang in BENCHMARK_LANGUAGES:
        lang_results = os.path.join(benchmark, "results", lang, "rapl.csv")
        if not os.path.exists(lang_results):
            print(f"[WARNING] Missing compiled language results '{lang_results}'. Skipping.")
            continue
        ...
        for bench in BENCHMARKS:
            if bench not in list(lang_df["Algorithm"]):
                print(f"[WARNING] Missing benchmark '{bench}' in '{lang_results}'. Skipping.")
                continue
...
def interactive_caches(benchmark, step=1):
    plots_dir = os.path.join(benchmark, "results", "plots", "interactive_caches")
    if not os.path.exists(plots_dir):
        os.makedirs(plots_dir)
    ...
    for lang in BENCHMARK_LANGUAGES:
        ...
        for bench in BENCHMARKS:
            bench_results = os.path.join(benchmark, "results", lang, bench, "cache.txt")
            ...
            average_cache_misses = None
            if "LLC Load Misses" in metrics:
                average_cache_misses = sum(metrics["LLC Load Misses"]) / len(
            ...
            for pkg_end, dram_end, time_end in zip(pkg, dram, time):
                ...
                if time_sum <= max_timestamp:
                    fig.add_vline(
                        ...
                        annotation_text=f"PKG: <b>{pkg_end}</b><br>DRAM: <b>{dram_end}</b><br>TIME: <b>{time_end}</b>",
                        annotation_position="top left"
                    )

            # Add average line and annotation
            if average_cache_misses is not None:
                fig.add_hline(
                    y=average_cache_misses,
                    line=dict(color="blue", dash="dot"),
                    annotation_text=f"Average: {average_cache_misses:.2f}",
                    annotation_position="top left",
                )

            # Customize layout
            fig.update_layout(
                title=f"Interactive Cache Metrics Over Time: {lang} - {bench}",
...
def caches(benchmark, step=1):
    plots_dir = os.path.join(benchmark, "results", "plots", "caches")
    if not os.path.exists(plots_dir):
        os.makedirs(plots_dir)
...
def parse_args():
    ...
    parser_warmup = subparsers.add_parser("warmup", help="")
    parser_warmup.add_argument("-a", "--average-results", action="store_true", help="")
    parser_warmup.add_argument(
        "-n", "--normalize-results", action="store_true", help=""
    )
    parser_warmup.add_argument("-d", "--dendrograms", action="store_true", help="")
    parser_warmup.add_argument("-v", "--violins", action="store_true", help="")
    parser_warmup.add_argument("-c", "--caches", action="store_true", help="")
    parser_warmup.add_argument(
        "-i", "--interactive-caches", action="store_true", help=""
    )

    parser_no_warmup = subparsers.add_parser("no-warmup", help="")
    parser_no_warmup.add_argument(
        "-a", "--average-results", action="store_true", help=""
    )
    parser_no_warmup.add_argument(
        "-n", "--normalize-results", action="store_true", help=""
    )
    parser_no_warmup.add_argument("-d", "--dendrograms", action="store_true", help="")
    parser_no_warmup.add_argument("-v", "--violins", action="store_true", help="")
    parser_no_warmup.add_argument("-c", "--caches", action="store_true", help="")
    parser_no_warmup.add_argument(
        "-i", "--interactive-caches", action="store_true", help=""
    )

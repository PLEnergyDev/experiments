...
void rapl_before(FILE *fp, int core) {
    ...
    if ((cpu_model == CPU_SANDYBRIDGE_EP) || (cpu_model == CPU_IVYBRIDGE_EP)) {
        ...
        // fprintf(fp,"Accumulated Package Throttled Time :
        // %.6fs\n",acc_pkg_throttled_time);
    }

    result = read_msr(fd, MSR_PP0_ENERGY_STATUS);
    pp0_before = (double)result * energy_units;
    // fprintf(fp,"PowerPlane0 (core) for core %d energy before:
    // %.6fJ\n",core,pp0_before);
    ...
    if ((cpu_model == CPU_SANDYBRIDGE_EP) || (cpu_model == CPU_IVYBRIDGE_EP)) {
        ...
        // fprintf(fp,"PowerPlane0 (core) Accumulated Throttled Time :
        // %.6fs\n",acc_pp0_throttled_time);
    }

    result = read_msr(fd, MSR_PP1_ENERGY_STATUS);
    pp1_before = (double)result * energy_units;
    // fprintf(fp,"PowerPlane1 (on-core GPU if avail) before:
    // %.6fJ\n",pp1_before);
    result = read_msr(fd, MSR_PP1_POLICY);
    int pp1_policy = (int)result & 0x001f;
    // fprintf(fp,"PowerPlane1 (on-core GPU if avail) %d policy:
    // %d\n",core,pp1_policy);
